...
def add_course_api(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"id": course.id, "coursename": course.coursename}, status=201
            )
...
def get_course_api(request, course_id):
    if request.method == "GET":
        try:
            ...
            return JsonResponse(
                {"id": course.id, "coursename": course.coursename}, status=200
            )
        except Exception:
...
def list_courses_api(request):
    if request.method == "GET":
        try:
            ...
            data = [
                {"id": course.id, "coursename": course.coursename} for course in courses
            ]
...
def update_course_api(request, course_id):
    if request.method in ["PUT", "PATCH"]:
        try:
            ...
            return JsonResponse(
                {"error": "Course not found or update failed."}, status=404
            )
